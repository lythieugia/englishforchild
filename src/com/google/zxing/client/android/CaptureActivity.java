...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback {
	...
	private LinearLayout lnSpeech2Text;
	private final int REQ_CODE_SPEECH_INPUT = 100;
	// get Server Data
	
	private String mURL = HttpHelper.TAG_HOST + HttpHelper.TAG_GET_GROUP_WORD;
	private List<NameValuePair> mParam;
	private String mImageId;
	private String mWord = "";
	private LinearLayout lnCong;
	private Button btnDone;
	private Boolean isWorking = false;
	private int mWrongSpeakNumber = 0;
	private int mCurrentUserId = -1;
	private ArrayList<ResultWord> mArrResult;
	
	//data
	private Group mWordGroup;
	...
	public void onCreate(Bundle icicle) {
		...
		// Speech to text
		lnSpeech2Text = (LinearLayout) findViewById(R.id.lnSpeech2Text);
		...
		lnCong = (LinearLayout) findViewById(R.id.lnCong);
				...
				if (resultCode == RESULT_OK && null != intent) {
					...
					if (isSpeakingCorrect(result.get(0), mWord)) {
						Toast.makeText(getApplicationContext(), "Correct! The word is  " + mWord, Toast.LENGTH_SHORT)
								.show();
						playSound();
						lnCong.setVisibility(View.VISIBLE);
						lnSpeech2Text.setVisibility(View.GONE);
						setPassedForWord(mWord);
						...
						mWrongSpeakNumber++;
						Toast.makeText(getApplicationContext(), "Incorrect! \r\n You've mispronounced "
								+ mWrongSpeakNumber + " times.\r\n Please try again", Toast.LENGTH_SHORT).show();
						setWrongTimeForWord(mWord, mWrongSpeakNumber);
					}
				}
				break;
			}
	...
	private void handleDecodeInternally(Result rawResult, ResultHandler resultHandler, Bitmap barcode) {
		...
		if (displayContents != "null") {		
			...
			Log.e("DECODE_RESULT", "url "+mImageId);
			mWord = getWordByGroupImageId(mImageId);
			Log.e("DECODE_RESULT", "word "+mWord);
			if (textIsNotEmpty(mWord)) {
				if (checkPassedGroup(mWord)) {
					...
					lnSpeech2Text.setVisibility(View.VISIBLE);
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					String serverUrl = HttpHelper.TAG_HOST + HttpHelper.TAG_SEND_RESULT;
	...
	private void resetStatusView() {
		...
		lnCong.setVisibility(View.GONE);
		lastResult = null;
	}
	...
	Handler getWordHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == HttpHelper.LOAD_WORD_DONE) {
				...
				Toast.makeText(getApplicationContext(), "This QRCode is invail. Please check again!", Toast.LENGTH_LONG)
						.show();
				if (source == IntentSource.NATIVE_APP_INTENT) {
					setResult(RESULT_CANCELED);
					finish();
			...
			if ((msg.what == HttpHelper.LOAD_WORD_NETWORD_ERROR)) {
	...
	private String getWordByGroupImageId( String imageId) {
		ArrayList<Word> listWord = mWordGroup.getWordList();	
			if (listWord != null && listWord.size() > 0) {
				for (int j = 0; j < listWord.size(); j++) {
					if (listWord.get(j).getImageId().equals(imageId)) {
						return listWord.get(j).getWord();
					}
				}
			}
	...
	private String getImageByGroupImageId(String imageId) {
		ArrayList<Word> listWord = mWordGroup.getWordList();
		if (listWord != null && listWord.size() > 0) {
			for (int j = 0; j < listWord.size(); j++) {
				if (listWord.get(j).getImageId().equals(imageId)) {
					return listWord.get(j).getImageFile();
				}
			}
		}
		return null;
	...
	public Integer getGroupIndexByWord(String word) {
		ArrayList<Word> listWord = mWordGroup.getWordList();
		if (listWord != null && listWord.size() > 0) {
			for (int j = 0; j < listWord.size(); j++) {
				if (listWord.get(j).getWord().equals(word)) {
					return j;
				}
			}
		}
	...
	private void setWrongTimeForWord(String word, int wrongtime){
		ArrayList<Word> list = mWordGroup.getWordList();
		for(int i = 0; i<list.size();i++){
			if(list.get(i).getWord().equals(word)){
				mWordGroup.getWordList().get(i).setWrongCount(wrongtime); 
				break;
			}
		}
	}
	
	public boolean textIsNotEmpty(String text) {
		if (text != null && !text.isEmpty() && text.trim().length() > 0 && !text.equals("null")
				&& !text.equals("NULL")) {
			return true;
		}
		return false;
	...
	private Boolean checkPassedGroup(String word) {
		ArrayList<Word> list = mWordGroup.getWordList();
		for (int i = 0; i < list.size(); i++) {
	...
	private boolean checkDataGroupIsNotNull(int pId) {
		if (GROUP_LIST.size() > 0) {
			for (int i = 0; i < GROUP_LIST.size(); i++) {
				if (GROUP_LIST.get(i).getGroupId() == pId && GROUP_LIST.get(i).getWordList() != null
						&& GROUP_LIST.get(i).getWordList().size() > 0) {
					return true;
				}
			}
		}
	...
	private void setPassedForWord(String word) {
		Integer pos = getGroupIndexByWord(word);
		ArrayList<Word> list = mWordGroup.getWordList();
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getWord().equals(word)) {
	...
	private void playSound() {
		...
		try {
			AssetFileDescriptor descriptor = CaptureActivity.this.getAssets().openFd("tada.mp3");
